...
## Vulkan DLL Version
Download and run `Install-RandOverlay.bat` once. Keep the Archipelago Text Client active while playing. This version draws inside the emulator frame, so it is the one to use for exclusive fullscreen play — the AHK and PowerShell + WPF editions may not display correctly in fullscreen.
